...
string TEditor::writeString(string otherStr) {
	/*if (otherStr.size() < 3)
		return frac;
	int posOfSep = otherStr.find(separator);
	if ((posOfSep == -1) || (posOfSep == 0) || (posOfSep == otherStr.size() - 1))
		return frac;
	if (!checkForNumbers(otherStr.substr(0, posOfSep - 1)) || !checkForNumbers(otherStr.substr(posOfSep + 1, otherStr.size()))){
		return frac;
	}
	if (otherStr.substr(0, posOfSep - 1) != "0") {
		while(otherStr[0] == '0')
			otherStr.erase(0, 1);
		int posOfSep = otherStr.find(separator);
	}
	if (otherStr.substr(posOfSep + 1, otherStr.size()) != "0") {
		while(otherStr[posOfSep + 1] == '0')
			otherStr.erase(0, 1);
	}*/
